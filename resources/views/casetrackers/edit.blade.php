...
<div class="iq-card-body">
    <form method="POST" action="{{ route('casetrackers.updatecasetrackers',$fileid) }}" enctype="multipart/form-data">
        ...
        <div class="col-sm-12" style="width:100%;margin: 20px;" id="form">
            ...
            <input type="hidden" name="caseresponses[{{$loop->index}}][id]" value="{{$caseresponse->id}}" disabled="true">
            ...
            <div class="form-group">
                ...
                <input type="text" aria-describedby="response" name="caseresponses[{{$loop->index}}][response]" value="{{$caseresponse->response}}" disabled="true">
            </div>
            @break
            ...
            <div class="form-group">
                <label for="response"> {{$caseresponse->questions->questionid}} : {{$caseresponse->questions->question}}</label>
                <input type="date" aria-describedby="response" name="caseresponses[{{$loop->index}}][response]" value="{{$caseresponse->response}}" disabled="true">
            </div>
            @break
            ...
            <div class="form-group">
                <label for="response"> {{$caseresponse->questions->questionid}} : {{$caseresponse->questions->question}}</label>
                <input type="file" aria-describedby="response" name="caseresponses[{{$loop->index}}][response]" value="{{$caseresponse->response}}" disabled="true">
